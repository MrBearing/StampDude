#include "stamp_dude/point_to_point_stamped.hpp"
#include "gtest/gtest.h"
#include "rclcpp/rclcpp.hpp"
#include "geometry_msgs/msg/point.hpp"
#include "geometry_msgs/msg/point_stamped.hpp"
#include <chrono>
#include <thread>

class PointToPointStampedTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
    if (!rclcpp::ok()) {
      rclcpp::init(0, nullptr);
    }
    rclcpp::NodeOptions options;
    node_ = std::make_shared<stamp_dude::PointToPointStamped>(options);
    
    // Create publisher to send messages to the node
    pub_ = node_->create_publisher<geometry_msgs::msg::Point>("point", 10);
    
    // Create subscription to receive stamped messages from the node
    sub_ = node_->create_subscription<geometry_msgs::msg::PointStamped>(
      "point_stamped", 10,
      [this](const geometry_msgs::msg::PointStamped::SharedPtr msg) {
        last_stamped_msg_ = msg;
      });
    
    executor_ = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
    executor_->add_node(node_);
  }
  
  void TearDown() override
  {
    node_.reset();
    executor_.reset();
  }
  
  std::shared_ptr<stamp_dude::PointToPointStamped> node_;
  rclcpp::Publisher<geometry_msgs::msg::Point>::SharedPtr pub_;
  rclcpp::Subscription<geometry_msgs::msg::PointStamped>::SharedPtr sub_;
  geometry_msgs::msg::PointStamped::SharedPtr last_stamped_msg_;
  std::shared_ptr<rclcpp::executors::SingleThreadedExecutor> executor_;
};

TEST_F(PointToPointStampedTest, InitializationTest)
{
  ASSERT_NE(nullptr, node_);
}

TEST_F(PointToPointStampedTest, MessageConversionTest)
{
  auto point_msg = geometry_msgs::msg::Point();
  point_msg.x = 1.0;
  point_msg.y = 2.0;
  point_msg.z = 3.0;
  
  pub_->publish(point_msg);
  
  // Wait for message to be processed
  auto start_time = std::chrono::steady_clock::now();
  while (!last_stamped_msg_ && 
         std::chrono::steady_clock::now() - start_time < std::chrono::seconds(1)) {
    executor_->spin_once(std::chrono::milliseconds(10));
  }
  
  ASSERT_NE(nullptr, last_stamped_msg_);
  EXPECT_DOUBLE_EQ(1.0, last_stamped_msg_->point.x);
  EXPECT_DOUBLE_EQ(2.0, last_stamped_msg_->point.y);
  EXPECT_DOUBLE_EQ(3.0, last_stamped_msg_->point.z);
  
  // Check that timestamp is set
  auto stamp_sec = last_stamped_msg_->header.stamp.sec;
  auto stamp_nanosec = last_stamped_msg_->header.stamp.nanosec;
  EXPECT_GE(stamp_sec, 0);
  EXPECT_GE(stamp_nanosec, 0u);
}

TEST_F(PointToPointStampedTest, MultipleMessagesTest)
{
  auto point_msg1 = geometry_msgs::msg::Point();
  point_msg1.x = 1.0;
  point_msg1.y = 2.0;
  point_msg1.z = 3.0;
  
  pub_->publish(point_msg1);
  
  // Wait for first message
  auto start_time = std::chrono::steady_clock::now();
  while (!last_stamped_msg_ && 
         std::chrono::steady_clock::now() - start_time < std::chrono::seconds(1)) {
    executor_->spin_once(std::chrono::milliseconds(10));
  }
  
  ASSERT_NE(nullptr, last_stamped_msg_);
  auto first_stamp = last_stamped_msg_->header.stamp;
  
  std::this_thread::sleep_for(std::chrono::milliseconds(10));
  
  auto point_msg2 = geometry_msgs::msg::Point();
  point_msg2.x = 4.0;
  point_msg2.y = 5.0;
  point_msg2.z = 6.0;
  
  pub_->publish(point_msg2);
  
  // Wait for second message
  start_time = std::chrono::steady_clock::now();
  while (last_stamped_msg_->point.x != 4.0 && 
         std::chrono::steady_clock::now() - start_time < std::chrono::seconds(1)) {
    executor_->spin_once(std::chrono::milliseconds(10));
  }
  
  EXPECT_DOUBLE_EQ(4.0, last_stamped_msg_->point.x);
  EXPECT_DOUBLE_EQ(5.0, last_stamped_msg_->point.y);
  EXPECT_DOUBLE_EQ(6.0, last_stamped_msg_->point.z);
  
  auto second_stamp = last_stamped_msg_->header.stamp;
  EXPECT_GT(rclcpp::Time(second_stamp), rclcpp::Time(first_stamp));
}

int main(int argc, char **argv)
{
  testing::InitGoogleTest(&argc, argv);
  rclcpp::init(argc, argv);
  auto result = RUN_ALL_TESTS();
  rclcpp::shutdown();
  return result;
}